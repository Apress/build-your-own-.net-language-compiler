...
Public Class qbVariableType
    ...
    ' *         type is created by removing the toplevel lowerBound and the *
    ' *         upperBound. The returned variable type will be              *
    ' *         an array of one lower dimension, a variant, or a scalar.    *
    ' *                                                                     *
    ' *      *  The read-only property BoundList returns the list of array  *
    ' *         bounds for an array and a null string for a non-array.      *
    ' *                                                                     *
    ' *         This list is in the format LBound1,Ubound1,... for each pair*
    ' *         of upper and lower bounds from the highest to the lowest.   *
    ' *                                                                     *
    ...
    ' *         object's type.                                              *
    ' *                                                                     *
    ' *      *  The isNull method returns True (variable type is Null) or   *
    ' *         False (variable type is not Null.)                          *
    ' *                                                                     *
    ...
    ' *         Variant type (containing a random scalar value). Note that  *
    ...
    ' *                                                                     *
    ' *      *  The udtMemberType(i) method returns the type of a           *
    ' *         UDT's member (it raises an error when the object instance   *
    ' *         is not a UDT). Note that the type is returned as the        * 
    ' *         qbVariableType object for the type.                         *
    ' *                                                                     *
    ' *         The index i must be the integer index from 1 (and up to the *
    ' *         value of UDTmemberCount).                                   *
    ' *                                                                     *
    ...
    ' *         method.                                                     *
    ' *                                                                     *
    ' *      *  The read-only, Shared VariableTypeList property returns the *
    ...
    ' * and its maximum size, is available with a special parameter of the  *
    ' * object2XML method for converting the object state to an XML tag:    *
    ' * see the object2XML method for more information.                     *
    ' *                                                                     *
    ' ***********************************************************************
...
#End If
    ...
    '
    '
    Public ReadOnly Property BoundList() As String
        Get
            If Not checkUsable_("BoundList get") Then Return ""
    ...
    End Function
    ' --- Enforces strict type and usability: does a state clone for
    ' --- internal use only
    Friend Function clone(ByVal booStateClone As Boolean) As qbVariableType
        If Not checkUsable_("clone") Then Return (Nothing)
    ...
    Public Shared Function name2NetType(ByVal strSystemName As String) As String
        ...
        Catch ex As Exception
            errorHandler_("Can't create object: " & Err.Number & " " & Err.Description, _
                          ClassName, _
                          "name2NetType", _
                          "Returning a null string")
            Return ""
    ...
    Public Shared Function netType2Name(ByVal strNetType As String) As String
        ...
        Select Case Trim(UCase(strNetType))
            ...
            Case "OBJECT"
                objObject = New Object
                Return (objObject.GetType.ToString)
            Case Else
...
#End If
    ...
    ' -----------------------------------------------------------------------
    ' Return UDT member type as its fromString expression
    '
    '
    Public Function udtMemberType(ByVal objIndex As Object) As qbVariableType
        If Not checkUsable_("udtMemberType") Then Return (Nothing)
        If Not Me.isUDT Then
            errorHandler_("Object is not a UDT", "udtMemberType", "Returning nothing")
            Return (Nothing)
        End If
        Dim intIndex1 As Integer = udtIndex2Integer_(objIndex, Me.UDTmemberCount)
        If intIndex1 = 0 Then Return (Nothing)
        Return (CType(CType(CType(USRstate.objVarType, Collection).Item(intIndex1), _
                            Collection).Item(3), _
                      qbVariableType))
    End Function
    ...
    '
    '
    Private Function dimensionList_() As String
        With USRstate
