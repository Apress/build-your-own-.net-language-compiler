...
#Region " General procedures "
    ...
    Private Sub displayInfoForm(ByVal strUtility As String, _
        ...
        Dim frmUtilitiesInfo As utilitiesInfo
        Try
            frmUtilitiesInfo = New utilitiesInfo
        Catch ex As Exception
        ...
        Catch ex As Exception
            ...
            Return                         
        End Try     
        With frmUtilitiesInfo
            .txtAbstract.Text = usrIndexedProcedures.usrProcedures(intIndex1).strAbstract & _
                                ...
                                (usrIndexedProcedures.usrProcedures(intIndex1).strText, _
                                 "S O U R C E   C O D E")
            .Text = usrIndexedProcedures.usrProcedures(intIndex1).strName
            .txtAbstract.SelectionLength = 0
            .ShowDialog
            .Refresh
        End With            
    End Sub    
            ...
            ElseIf (Not .colUsedBy Is Nothing) AndAlso (Not .colUses Is Nothing) Then
                ...
                With .colUsedBy
                    For intIndex1 = 1 To .Count
                        _OBJutilities.append(strUsed, vbNewline, CStr(.Item(intIndex1)))
                    Next intIndex1
                End With                
                Dim strUses As String                                         
                With .colUses
                    For intIndex1 = 1 To .Count
                         _OBJutilities.append(strUses, vbNewline, CStr(.Item(intIndex1)))
                    Next intIndex1
                End With      
                strUsageAnalysis &= _
                    CStr(Iif(strUsed <> "", _
                             _OBJutilities.string2Box(strUsed, _
                                                      "This procedure is used by the following procedures"), _
                             "This procedure isn't used by any other procedure")) & _
                    vbNewline & vbNewline & _          
                    CStr(Iif(strUses <> "", _
                             _OBJutilities.string2Box(strUses, _
                                                      "This procedure uses the following procedures"), _
                             "This procedure does not use any other procedure"))
            Else
    ...
    Private Sub usageAnalysis(ByVal usrIndexedProcedures As TYPindexedProcedures)
        ...
        updateStatusListBox(strActivity, 1)
        Do 
            strActivity2 = "Iterating through all procedures to add indirect usage"
            updateStatusListBox(strActivity2, 1)
            booChange = False
            With usrIndexedProcedures
                For intIndex1 = 1 To UBound(.usrProcedures)
                                ...
                                With usrIndexedProcedures.usrProcedures(intIndex3).colUses
                                    For intIndex4 = 1 To .Count
                                        ...
                                        Catch
                                            booError = True
                                        End Try      
                                        If Not booError Then booChange = True                                  
                                    Next intIndex4        
                                End With       
    ...
    Private Sub zoomInterface(ByVal ctlZoomed As Control)
        ...
        Try
            ctlZoom = New zoom.zoom(ctlZoomed, CDbl(2), CDbl(3))
        Catch ex As Exception
